...
# set file logging
file_handler = logging.FileHandler('logs/solver_v3.log', mode='a')
...
def optimize_prices(Z, actual_demand_by_server_generation,
                            selling_prices,
                            elasticity,failure_rate,
                            init_selling_prices=None):
    '''
    Z shape (7, 168, 3)
    actual_demand_by_server_generation shape (7, 168, 3)
    selling_prices shape (7, 1, 3)
    elasticity shape (7, 1, 3)
    
    '''

    def optimize_selling_prices(x):
        new_selling_prices_matrix = x.reshape(actual_demand_by_server_generation.shape)
        delta_prices = (new_selling_prices_matrix - selling_prices)/selling_prices
        delta_demand_matrix = elasticity * delta_prices
        new_demand_matrix = actual_demand_by_server_generation * (1 + delta_demand_matrix)
        Z_new = Z * (1- failure_rate)
        Z_min = np.minimum(Z_new, new_demand_matrix)
        profit = (Z_min * new_selling_prices_matrix).sum(axis=(0,2))
        return -profit.sum()
    
    ## using scipy to optimize
    ## init x0
    length = actual_demand_by_server_generation.shape[0] * actual_demand_by_server_generation.shape[1] * actual_demand_by_server_generation.shape[2]
    if init_selling_prices is None:
        x0 = np.zeros(actual_demand_by_server_generation.shape)
    else:
        x0 = init_selling_prices.flatten()
    res = minimize(optimize_selling_prices, x0, method='l-bfgs-b',bounds=[(0,np.inf) for i in range(length)])
    new_selling_prices_matrix = res.x.reshape(actual_demand_by_server_generation.shape)
    return new_selling_prices_matrix
...
class Solver:
    ...
    def load_demand(self, demand):
        ...
        self.actual_demand_by_server_generation = np.asarray([actual_demand_by_server_generation[server_generation][['high', 'medium', 'low']] 
                                                              for server_generation in self.server_generation_unique])
        self.original_actual_demand = np.copy(self.actual_demand_by_server_generation)
    ...
    def update_demand_by_solution_selling_prices(self,):
        self.load_demand(self.df_demand) # reload actual_demand_by_server_generation
    ...
    def optimize_selling_prices(self,):
        new_selling_prices = optimize_prices(self.solution_Z, 
                                             actual_demand_by_server_generation=self.original_actual_demand,
                                             selling_prices=self.selling_prices,
                                             elasticity=self.price_elasticity_of_demand,
                                             failure_rate=self.failure_rate,
                                             init_selling_prices=self.solution_selling_prices)
        current_obj = self.solution_obj
        current_solution_selling_prices = self.solution_selling_prices
        self.solution_selling_prices = new_selling_prices
        self.update_demand_by_solution_selling_prices()
        new_obj, _ = self.calculate_new_obj(new_num_servers=self.solution_num_servers,
                                new_Z=self.solution_Z,
                                new_L=self.solution_L,
                                new_C=self.solution_C)

        if new_obj <= self.solution_obj:
            self.solution_selling_prices = current_solution_selling_prices
            self.update_demand_by_solution_selling_prices()
            return
        
        self.solution_obj = new_obj
        
        logging.info(f"Optimized selling prices with gain {new_obj - current_obj}")
    ...
    for _ in range(n_solving_loops):
        for step in range(4):
            if step == 0:
                ...
                solver.optimize_selling_prices()
